function [CSS,CSS_temp,necc_sleep_amount] = Calcul_CSS(no_sleep,coef_x, coef_y, v_vh, tau_c, gate, main_sleep, y1_1, t1_1, tspan2, real_sl_time, real_wk_time, real_sl_time_origin, real_wk_time_origin)
...
CSS_Dv = zeros(length(real_wk_time),1);                                      %Base for Dv at each sleep offset of 'all' sleep
CSS_temp = zeros(length(main_sleep),1);                                      %Base for sufficient or not for each main sleep (1='sufficient', 0='insufficient)  
CSS_temp2 = zeros(length(main_sleep),1);                                     %Base for Dv at each sleep offset of 'main' sleep

for j = 1 : length(real_wk_time)
    CSS_Dv(j) = D_v(real_wk_time(j));
end                                                                         %Calculate Dv at each sleep offset of 'all' sleep

for j = 1 : length(main_sleep)
    CSS_temp2(j) = CSS_Dv(main_sleep(j));
...
for j = 1 : length(main_sleep)
    if early_confirm(j) == 1
        ...
        if CSS_temp2(j) <= 2.46                                              %When sufficient sleep
...
for j = 1 : length(main_sleep)
    if j ~= length(main_sleep)
        if (CSS_temp(j) == 0) && (main_sleep(j) + 1 < main_sleep(j+1))
        ...
        if (CSS_temp(j) == 0) && (main_sleep(j) < length(real_sl_time))
...
CSS = 100 * sum(sum(CSS_temp)) / (length(main_sleep)+no_sleep);   
end