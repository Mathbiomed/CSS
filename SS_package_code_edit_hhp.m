%% SS_package (written by Jaehyoung Hong)
...
% Baseline light on
gating_time = 6;                                                            %Reasonable light on timing according to            
...
start_num = 10;                                                             %Choose 120 days which is usually enough to get stable sleep-wake patterns
...
for j = 1 : length(i_first)
    if it2(j) >= gating_time && it2(j) < 22-0.3
        i_first(j) = 250;
    end
...
real_wk_time = [];
real_sl_time = [];
for j = 1 : length(sleep)-1
    if sleep(j) + 1 < sleep(j+1)
        sleep_num = sleep_num + 1;
        real_wk_time = [real_wk_time; sleep(j)+1];
        real_sl_time = [real_sl_time; sleep(j+1)];
    end
end                                                                         %Find sleep-starting, finishing time, and #(sleep)
...
for j = 1 : length(real_sl_time)
    if ceil(waso(j,1)/time_interval) ~= 0
        patt(real_wk_time(j,1) - ceil(waso(j,1)/time_interval):real_wk_time(j,1),1) = 'Wake';
...
tspan_total = [];                                                           
for x = 1:length(tspan_first)
    tspan_total = [tspan_total;tspan_first(x)];
end
for x = 2:length(tspan2)
    tspan_total = [tspan_total;tspan2(x)];
end
for x = 2:length(tspan)
    tspan_total = [tspan_total;tspan(x,1)];
end                                                                         %Make total time
%HHP: you can just write this way 
%HHP: tspan_total = [tspan_first' ; tspan2(2:end)' ; tspan(2:end)]; save time!

i_total = [];
it_total = [];
for x = 1:length(it_first)
    i_total = [i_total;i_first(x)];
    it_total = [it_total;it_first(x)];
end
for x = 2:length(it2)
    i_total = [i_total;i2(x)];
    it_total = [it_total;it2(x)];
end
for x = 2:length(it)
    i_total = [i_total;light(x)];
    it_total = [it_total;it(x)];
end                                                                         %Make total light
%HHP: you can just write this way 
%HHP: i_total = [i_first' ; i2(2:end)' ; light(2:end)];
%HHP: it_total = [it_first' ; it2(2:end)' ; it(2:end)];


% Simulation at the gap between the end time of intial 120 days and the
% starting time of actigraphy 
V_0 =  [y0(length(y0),1); y0(length(y0),2); y0(length(y0),3); y0(length(y0),4); y0(length(y0),5); y0(length(y0),6)];
%HHP: you can just write this way
%HHP: V_0 = y0(end,:)';
...
wakeup = find(D_v <= 2.46); %HHP: here 2.46 or 1.45?
if isempty(wakeup) ~= 1
    if isempty(find(D_v(wakeup(1):end) > 2.46)) ~= 1
        sleep = find(D_v(wakeup(1):end) > 2.46);
        V_0 =  [y1_1(wakeup(1)+sleep(1)-1,1); y1_1(wakeup(1)+sleep(1)-1,2); y1_1(wakeup(1)+sleep(1)-1,3); wake_effort_Vv; wake_effort_Vm; y1_1(wakeup(1)+sleep(1)-1,6)];
        [t1_1(wakeup(1)+sleep(1)-1:end,1), y1_1(wakeup(1)+sleep(1)-1:end,:)] = ode15s(@(t,V) PCR_shift(t,V,it_total,i_total,tau_c,mu, v_vh, coef_x, coef_y, const, gate), tspan2(wakeup(1)+sleep(1)-1:end), V_0);       
    end
end
...
for day = 1 : length(st_fi)-1
    ...
    V_0 = [y1_1(length(y1_1),1); y1_1(length(y1_1),2); y1_1(length(y1_1),3); y1_1(length(y1_1),4); y1_1(length(y1_1),5); y1_1(length(y1_1),6)];   
    %HHP: you can just write this way
    %HHP: V_0 = y1_1(end,:)';
...
while(1)
    a = a + 1;
    matching_temp = Data_matching(day, coef_x, coef_y, v_vh, tau_c, gate, wake_effort_Vm, wake_effort_Vv, sleep_effort_Vm, sleep_effort_Vv,  y1_2, st_fi, patt, t1_2, tspan, real_sl_time, it_total,i_total);
...
if length(main_sleep) < day_new
   ...
   for  j = 1 : day_new
       ...
       if length(trick_day) == 2
           if (isempty(find(day_set==j-1)) == 1 & isempty(find(day_set==j+1)) == 0)
               day_set(trick_day(1)) = day_set(trick_day(1)) - 1;
           elseif (isempty(find(day_set==j-1)) == 0 & isempty(find(day_set==j+1)) == 1)
   ...
   for j = 1 : day_new
      if (isempty(find(day_set==j)) == 1)
        no_sleep = [no_sleep; j];
      end
   end
end

[SS, SS_temp,normal_sleep_amount2] = Calcul_SS(length(no_sleep), coef_x, coef_y, v_vh, tau_c, gate, main_sleep, y1_1, t1_1, tspan, tspan2, real_sl_time, real_wk_time, real_sl_time_origin, real_wk_time_origin);
                                                                            %SS calculation
...
nece_sleep = normal_sleep_amount2;
temp = SS_temp;
suff = string(temp);
SS_change = zeros(length(main_sleep),1);
...
for j = 1 : length(SS_change)
    no_sleep_day = length(find(no_sleep <= j));                             %Find number of no_sleep_day that must be insufficiet sleep 
    SS_change(j) = 100*sum(SS_temp(1:j))/(j+no_sleep_day);                  
end
 
writematrix([nece_sleep, actual_sleep,suff,SS_change],[pwd '/Output/SS.csv'])
 
normal_time = normal_sleep_amount2;
...
figure(3)
for j = 1 : length(real_sl_time)   
    find_day = day_new-2-floor((sleep_interval(j)-(24-sleep_time))/24);         %Find y-axis
    find_time = mod(sleep_time + sleep_interval(j),24);                     %Find x-axis
    if find_time + duration_real(j) <= 24
        rectangle('Position',[find_time day_thick*find_day+bar_thick+day_diff_bot duration_real(j) bar_thick],'Facecolor',real_color,'Edgecolor',[1,1,1])
        hold on                                                             %Draw actual sleep
    else
        rectangle('Position',[find_time day_thick*find_day+bar_thick+day_diff_bot 24-find_time bar_thick],'Facecolor',real_color,'Edgecolor',[1,1,1])
        hold on                                                             %If wake after 17:00 draw remaining sleep on next day
        rectangle('Position',[0 day_thick*(find_day-1)+bar_thick+day_diff_bot duration_real(j)-(24-find_time) bar_thick],'Facecolor',real_color,'Edgecolor',[1,1,1])
        hold on
    end
    if main_index <= length(main_sleep)                                     %If we have more calculated necessary sleep, draw it
        if main_sleep(main_index) == j                                      %Draw necessary sleep if main sleep episode
            if find_time + duration_model(main_index) <= 24
                rectangle('Position',[find_time day_thick*find_day+day_diff_bot duration_model(main_index) bar_thick],'Facecolor',model_color,'Edgecolor',[1,1,1])
                hold on                                                     %Draw actual sleep
                main_index = main_index + 1;
            else
                rectangle('Position',[find_time day_thick*find_day+day_diff_bot 24-find_time bar_thick],'Facecolor',model_color,'Edgecolor',[1,1,1])
                hold on                                                     %If wake after 17:00 draw remaining sleep on next day
                rectangle('Position',[0 day_thick*(find_day-1)+day_diff_bot duration_model(main_index)-(24-find_time) bar_thick],'Facecolor',model_color,'Edgecolor',[1,1,1])
                hold on
                main_index = main_index + 1;
            end
        end
