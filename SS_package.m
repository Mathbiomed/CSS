%% SS_package (written by Jaehyoung Hong and Hyukpyo Hong)
...
[SS, SS_temp,normal_sleep_amount] = Calcul_SS(length(no_sleep)+base_of_sleep, coef_x, coef_y, v_vh, tau_c, gate, main_sleep, y1_1, t1_1, tspan2, real_sl_time, real_wk_time, real_sl_time_origin, real_wk_time_origin);
                                                                            %SS calculation
...
for j = 1 : length(SS_change)
    ...
    SS_change(j) = 100*sum(SS_temp(1:j))/(j+no_sleep_day+base_of_sleep);                  
end
